# Copyright 2023 Yunseong Hwang
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# SPDX-License-Identifier: Apache-2.0

from __future__ import annotations

import atexit
import ctypes
import ctypes.util
import logging
import os
import signal
import time
import weakref

from typing import TYPE_CHECKING

from axserve.common.runnable_process import RunnableProcess


if TYPE_CHECKING:
    from collections.abc import Callable


libc = ctypes.CDLL(ctypes.util.find_library("c"), use_errno=True)
logger = logging.getLogger(__name__)


PR_SET_PDEATHSIG = 1


def set_pdeathsig(
    sig: int = signal.SIGTERM,
):
    if libc.prctl(PR_SET_PDEATHSIG, sig) != 0:
        e = ctypes.get_errno()
        raise OSError(e, os.strerror(e))


def kill_process_group(
    pgid: int,
    sig: int = signal.SIGTERM,
    grace: float = 5.0,
):
    try:
        os.killpg(pgid, sig)  # type: ignore
    except ProcessLookupError:
        return
    except PermissionError:
        return
    try:
        time.sleep(grace)
        os.killpg(pgid, signal.SIGKILL)  # type: ignore
    except ProcessLookupError:
        return
    except PermissionError:
        return


class ManagedProcess(RunnableProcess):
    def __init__(
        self,
        *args,
        preexec_fn: Callable[[], None] | None = None,
        death_signal: int = signal.SIGTERM,
        **kwargs,
    ):
        if not kwargs.get("start_new_session", False):
            logger.warning(
                "%s: start_new_session was set to False, "
                "but will be forcibly overridden to True "
                "to ensure process group isolation.",
                self.__class__.__name__,
            )

        def _combined_preexec():
            if preexec_fn:
                preexec_fn()
            set_pdeathsig(death_signal)

        kwargs["start_new_session"] = True
        kwargs["preexec_fn"] = _combined_preexec

        super().__init__(*args, **kwargs)

        atexit.register(
            kill_process_group,
            os.getpgid(self.pid),  # type: ignore
            death_signal,
        )


class ScopedProcess(ManagedProcess):
    def __init__(
        self,
        *args,
        preexec_fn: Callable[[], None] | None = None,
        death_signal: int = signal.SIGTERM,
        **kwargs,
    ):
        super().__init__(
            *args,
            preexec_fn=preexec_fn,
            death_signal=death_signal,
            **kwargs,
        )

        weakref.finalize(
            self,
            kill_process_group,
            os.getpgid(self.pid),  # type: ignore
            death_signal,
        )
